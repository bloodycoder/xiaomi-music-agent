...
def _call_pause():
    r = requests.get('http://127.0.0.1:8765/pause', timeout=30)
    r.raise_for_status()
    return r.text


async def smartpause():
    global log
    did = await _cancel_xiaomusic_local_loop('smartpause')
    ...
    await _suppress_xiaoai_once(did, 'smartpause')
    text = await asyncio.to_thread(_call_pause)
    log.info(f'smartpause response:{text}')
