...
def _call_next():
    r = requests.get('http://127.0.0.1:8765/next', timeout=30)
    r.raise_for_status()
    return r.text
...
async def smartnext():
    global log
    did = await _cancel_xiaomusic_local_loop('smartnext')
    ...
    await _suppress_xiaoai_once(did, 'smartnext')
    text = await asyncio.to_thread(_call_next)
    log.info(f'smartnext response:{text}')
